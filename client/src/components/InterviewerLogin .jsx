...
export default function InterviewerLoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:5000/login/interviewer", {
        method: "POST",
        headers: {
      ...
      localStorage.setItem('interviewer', JSON.stringify(result.user))
      console.log(result);
      if (result.token) 
          ...
          <div className="space-y-6">
            ...
            <div className="text-center space-y-4">
              <div>
                <a
                  href="#"
                  className="text-gray-600 underline hover:text-gray-900 text-md"
                >
                  Forgot password?
                </a>
              </div>
